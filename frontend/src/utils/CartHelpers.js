...
export const getActiveConfiguredItems = (ConfiguredItems, cartAttribute) => {
   ...
   for (let i = 0; i < ConfiguredItems.length; i++) {
      ...
      const newConfig = Configurators?.filter(filter => CartAttributes?.find((findAttr) => findAttr.Pid === filter.Pid && findAttr.Vid === filter.Vid)) || [];
   ...
   if (configItems.length === 0) {
      configItems = ConfiguredItems?.filter(ConfiguredItem => ConfiguredItem?.Configurators?.find(find => CartAttributes?.find((findAttr) => findAttr.Pid === find.Pid && findAttr.Vid === find.Vid))) || [];
   }
   return configItems;
};


export const firstAttributeConfiguredItems = (ConfiguredItems, firstAttribute) => {
   return ConfiguredItems?.filter(findConfig => findConfig?.Configurators?.find(find => find.Pid === firstAttribute.Pid && firstAttribute.Vid === find.Vid)
   );
};
...
export const getProductModifiedConfiguredItem = ($product, qty, activeConfiguredItem, rate) => {
   let configItem = {};
   ...
   Configurators?.map(config => {
      const findAttr = Attributes.find(find => find.Pid === config.Pid && find.Vid === config.Vid);
      if (findAttr) {
...
export const getProductGroupedAttributes = Attributes => {
   const groupBy = Attributes?.length ? _.groupBy(Attributes, "PropertyName") : {};
   const returnGroup = [];
   for (const property in groupBy) {
      let attrItem = {};
      attrItem.key = property;
      attrItem.values = groupBy[property];
      returnGroup.push(attrItem);
   }
   return returnGroup.sort((a, b) => (a.key > b.key ? 1 : -1));
};
...
export const getDBProductPrice = (product, rate = 15, selectedConfig = {}) => {
   ...
   if (product?.Price) {
      ...
      if (Price?.length > 0) {
         sellPrice = Price[0]?.OriginalPrice ? Number(Price[0]?.OriginalPrice) * Number(rate) : sellPrice;
      }
...
export const getProductDeliveryCost = (product, rate = 15) => {
   ...
   if (!_.isEmpty(product) && _.isObject(product)) {
      ...
      if (_.isArray(DeliveryCosts)) {
         const DeliveryCost =
            DeliveryCosts.length > 0 ? DeliveryCosts[0] : {};
         if (!_.isEmpty(DeliveryCost)) {
            const Price = DeliveryCost.Price;
            const OriginalPrice = !_.isEmpty(Price)
               ? Price.OriginalPrice
               : 0;
...
export const getProductFeaturedValues = (product, keyName, common = 0) => {
   if (!_.isEmpty(product) && _.isObject(product)) {
      ...
      if (!_.isEmpty(FeaturedValues) && _.isArray(FeaturedValues)) {
         const FeatureItem = FeaturedValues.find(
            find => find.Name === keyName
         );
...
export const getCartSelectedConfig = ConfiguredItems => {
   if (!_.isEmpty(ConfiguredItems) && _.isArray(ConfiguredItems)) {
      const returnKey =
         ConfiguredItems.length === 1 ? ConfiguredItems[0] : {};
...
export const activeProductAllConfigurators = activeCartProduct => {
   if (isObject(activeCartProduct)) {
      return !isEmpty(activeCartProduct)
...
export const checkExistConfiguredItem = (
   ...
   const activeConfiguredItems = activeProductAllConfigurators(
      activeCartProduct
   );
...
export const CartProductSummary = (cart, advanced_rate) => {
   let totalPrice = 0;
   let shipping = 0;
   let totalQty = 0;
   let advanced = 0;
   ...
   if (totalPrice > 0) {
      ...
      advanced = Math.round((totalPrice * advanced_rate) / 100);
