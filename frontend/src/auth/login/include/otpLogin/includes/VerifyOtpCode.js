...
const VerifyOtpCode = (props) => {
	...
	const {registerSubmit: {mutateAsync, isLoading}} = useAuthMutation();

	const registerCustomer = (e) => {
		e.preventDefault();
		mutateAsync({phone, otp, email})
			.then((response) => {
				if(response?.id){
	...
	return (
		<div>
			...
			<p className="text-center">
				We just sent you a SMS with an OTP code on your <strong>{phone || email}</strong> as a login account, Please enter the 4-digit OTP
				code below.
			</p>
