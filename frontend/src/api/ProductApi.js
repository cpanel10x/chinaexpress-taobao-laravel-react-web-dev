...
export const useNewArrivedProducts = (page = 1) => useQuery(["useNewArrivedProducts", page], async () => {
	try {
		const {data} = await instance.get(`new-arrived-products`, {params: {page}});
		return data;
	} catch (error) {
		console.log(error);
	}
...
export const useSectionProducts = (section) => useQuery(['section', section], async () => {
	try {
		const {data} = await instance.get(`get-section-products`, {params: {section}});
