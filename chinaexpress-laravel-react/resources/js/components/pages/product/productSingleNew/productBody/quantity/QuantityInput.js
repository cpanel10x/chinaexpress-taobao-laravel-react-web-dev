...
import {addToCustomerCart} from "../../../../../api/Queries";
...
const QuantityInput = props => {
	...
	const {mutateAsync} = addToCustomerCart();
	...
	const addToCartProcess = (e) => {
		...
		if (activeConfiguredItem?.Id) {
			if (activeConfiguredItem?.Quantity > 0) {
				...
				toast.info('This variations stock is not available');
			}
		} else {
			if (ConfiguredItems?.length) {
				toast.info('Select your Item variations');
			} else {
				process = true;
	...
	if (!configItem || !cartConfigQty) {
		return (
			<AddToCartButton addToCartProcess={addToCartProcess} Quantity={Quantity} MasterQuantity={MasterQuantity}/>);
	}
