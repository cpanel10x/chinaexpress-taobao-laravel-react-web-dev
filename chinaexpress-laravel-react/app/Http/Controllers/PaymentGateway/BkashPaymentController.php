...
class BkashPaymentController extends Controller
{
    ...
    {
        $user_id = auth()->id();
        $order = Order::with('orderItems')->where('user_id', $user_id)->where('transaction_id', $id)->first();
        $amount = $order ? (int)($order->orderItems->sum('first_payment')) : 0;
        if (!$order) {
            $invoice = Invoice::where('user_id', $user_id)->where('transaction_id', $id)->first();
