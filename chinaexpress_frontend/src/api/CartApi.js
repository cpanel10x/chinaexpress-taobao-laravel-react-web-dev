...
export const addItemToWishList = async (processData) => {
	try {
		const {data} = await instance.post(`/add-to-wishlist`, processData, {withCredentials: true});
		return data;
	} catch (error) {
		throw Error(error.response.statusText);
	}
...
export const removeItemFromWishList = async (processData) => {
	try {
		const {data} = await instance.post(`/remove-wishlist`, processData, {withCredentials: true});
		return data;
	} catch (error) {
		throw Error(error.response.statusText);
	}
...
export const shippingAddress = async () => {
	const token = setGetToken();
	try {
		const {data} = await instance.post(`/address`, {token: token}, {withCredentials: true});
...
export const saveShippingAddressToCart = async (processData) => {
	const token = setGetToken();
	try {
		const {data} = await instance.post(`/cart/shipping`, {token: token, ...processData}, {withCredentials: true});
		return data;
	} catch (error) {
		throw Error(error.response.statusText);
	}
...
export const storeShippingAddress = async (processData) => {
	const token = setGetToken();
	try {
		const {data} = await instance.post(`/store-new-address`, {token: token, ...processData}, {withCredentials: true});
		return data;
	} catch (error) {
		throw Error(error.response.statusText);
	}
